...
export async function getContactByArabicName(arabicName) {
    const list = await listContacts();
    const contact = list.find(contact => contact.arabicName === arabicName);
    return contact || null;
}
...
export async function addContact(arabicName, englishName) {
    const list = await listContacts();
